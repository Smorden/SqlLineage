...
import com.siheng.cn.admin.entity.JobLog;
			...
			public void run() {
				...
				while (!toStop) {
					try {
						...
						if (!toStop) {
							logger.error(">>>>>>>>>>> LarkMidTable, job fail monitor thread error:{0}", e);
						}
					}
				...
				logger.info(">>>>>>>>>>> LarkMidTable, job fail monitor thread stop");

			}
		});
		monitorThread.setDaemon(true);
		monitorThread.setName("LarkMidTable, admin JobFailMonitorHelper");
		monitorThread.start();
	}
		...
		if (info!=null && info.getAlarmEmail()!=null && info.getAlarmEmail().trim().length()>0) {
			...
			for (String email: emailSet) {
				...
				try {
					...
					logger.error(">>>>>>>>>>> LarkMidTable, job fail alarm email send error, JobLogId:{}", jobLog.getId(), e);
