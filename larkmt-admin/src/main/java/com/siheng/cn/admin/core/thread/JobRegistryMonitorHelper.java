...
import com.siheng.core.enums.RegistryConfig;
		...
		registryThread = new Thread(() -> {
			while (!toStop) {
				try {
					...
					if (!toStop) {
						logger.error(">>>>>>>>>>> LarkMidTable, job registry monitor thread error:{}", e);
					}
				}
				try {
			...
			logger.info(">>>>>>>>>>> LarkMidTable, job registry monitor thread stop");
		});
		registryThread.setDaemon(true);
		registryThread.setName("LarkMidTable, admin JobRegistryMonitorHelper");
